VideoPlayerApp.controller('watchPageController', function ($scope, $rootScope, $http, $location, $sce, $timeout, $route) {
    ...
    $scope.loadUserPlaylist = function () {
        if ($scope.userToken !== null) {
            $http({
                ...
                $scope.playlistArray = [{
                    "attributes": {
                        "name": "Playlist thành công",
                        'thumbnailUrl': "https://pbs.twimg.com/profile_images/735499326936338432/y0FMUpYW.jpg"
                    },
                    "id": "0"
                }];
    ...
    // Edit
        $scope.editVideo = function () {
            if ($scope.editData.data.attributes.thumbnail === "") {
                $scope.editData.data.attributes.thumbnail = 'https://pbs.twimg.com/profile_images/735499326936338432/y0FMUpYW.jpg' + $scope.editData.data.attributes.youtubeId + '/mqdefault.jpg';
            }
            $http({
                method: 'PUT',
                headers: {
    ...
    // Delete
        ...
        $scope.addVideoToPlaylist = function () {
            if ($scope.editData.data.attributes.thumbnail === "") {
                $scope.editData.data.attributes.thumbnail = 'https://pbs.twimg.com/profile_images/735499326936338432/y0FMUpYW.jpg' + $scope.editData.data.attributes.youtubeId + '/hqdefault.jpg';
            }
            $http ({
                method: 'POST',
                headers: {
